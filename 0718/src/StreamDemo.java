...
/*
    ...
    5. 중간 연산의 결과가 Stream으로 리턴이 되기 때문에, Stream 연산을 게속 이어서 chain 형태로 이용할 수 있다.
    6. But, 최종연산의 결과는 Stream이 아니기 때문에 일단 최종연산을 수행하면 다시 다른 Stream 연산을 수행할 수 없다. (Consuming 됐다라고 표현)
    7. Stream 연산은 원본의 데이터를 읽어서 기능을 수행하지만, 그 내용을 변경하지는 않는다.
 */
public class StreamDemo {
    public static void main(String[] args) {
        //Stream 생성: 배열을 이용할 것인가, Collection을 이용할 것인가에 따라 생성 방법이 다르다.
        //배열로 부터 Stream 생성하기
        ...
        IntStream  is = IntStream.range(1,11); // 1부터 10까지 (시작번호, 끝번호+1)
        DoubleStream ds = DoubleStream.of(10.5); //10.5까지
